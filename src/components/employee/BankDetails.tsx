...
const BankDetails: React.FC<Props> = ({employees}) => {
    ...
    const updateBankDetails = async (formData:BankDetailsInterface) => {
        ...
        if(!formData.bankName || !formData.accountNumber || !formData.IFSCCode || !formData.PANNumber || !formData.UPIId) return
        const data: BankDetailsInterface = {... formData, userId: employee?._id}
        console.log(data);
