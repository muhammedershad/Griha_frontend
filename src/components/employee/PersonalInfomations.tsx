...
const PersonalInfomations: React.FC<Props> = ({ employee }) => {
    ...
    const updateEmployeeProfile = async (formData: Employees) => {
        ...
        if (!formData.firstName || !formData.lastName)
            toast.error("Enter valide first name and last name");
        if (
            !formData.username ||
            validations.validateUsername(formData.username).error
        ) {
            toast.error("Enter Valid username ");
        }
        if (!formData.email || validations.validateEmail(formData.email))
            toast.error("Enter valid email");
        if (!formData.phone || !validations.validatePhoneNumber(formData.phone))
